...
info "Welcome to The Fresh Cloud, we just need to setup some additional things... rev 0.010"
...
if [[ "$1" == "nami" && "$2" == "start" ]] || [[ "$1" == "httpd" ]]; then
    info "Doing some bitnami stuff ..."
    . /apache-init.sh
    . /wordpress-init.sh
    nami_initialize apache php mysql-client wordpress || true
    info "Doing some bitnami stuff completed"
fi
# this is from the original app-entrypoint.sh
...
chown -R daemon:daemon /bitnami/wordpress/
chown -R daemon:daemon /opt/bitnami/wordpress/
